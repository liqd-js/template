...
{
    #parser; #templates; #options;

    constructor( parser, templates, options = {})
    {
        this.#parser = parser;
        ...
        for( let node of nodes )
        {
            ...
            else if( node.style )
            {
                ...
                if( bundle )
                {
                    ...
                    bundles.styles[ bundle.value ].push( Style.compile( source, this.#options.style ));
                }
            }
            else if( node.if )
            {
    ...
    async bundle( options = {})
    {
        ...
        for( let bundle in options?.bundles )
        {
            ...
            for( let style of options.bundles[bundle]?.styles )
            {
                ...
                bundles.styles[ bundle ].push( Style.compile( fs.readFileSync( style, 'utf8' ), this.#options.style ));
            }
        }
        ...
        for( let bundle in bundles?.scripts )
        {
            ...
            /*try
            {
                source = require('@babel/core').transform( source, { presets: ['@babel/preset-env']}).code;
            }
            catch(e){}*/
