    ...
    static id( type, source )
    {
        if( source.path || ( source.code && source.code.trim() ))
        {
            ...
            if( !Cache[ type ].get( id ))
            {
                ...
                try
                {
                    source.code = require('@babel/core').transform( source.code, { presets: ['@babel/preset-env']}).code;
                }
                catch(e){}

                Cache[ type ].set( id, source.code );

                Resources.compress( type, source.code ).then( compressed => Cache[ type ].set( id, compressed )).catch( NOOP );
            }
