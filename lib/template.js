...
function concat( glue, ...parts )
{
    ...
    for( let part of parts )
    {
        if( part )
        {
            result += ( result.length ? glue : '' ) + part;
        }
    }
...
module.exports = class Template extends EventEmitter
{
    ...
    #transpile( parsed )
    {
        console.log( require('util').inspect( parsed, {depth: Infinity, colors:true} ));
    ...
    #render( template, options = {})
    {
        ...
        /* TODO undefined template -> render tag * /
        if( !template_render &&  )
        {
            // TODO args, promise only if necessary

            return Promise.all([`<${template.name  template}>`, options.content(), `</${template.name  template}>`]).then( r => r.join('') );
        }
        /* TODO END */
    ...
    async generate_bundles( options = {})
    {
        if( !this.#bundler )
        {
            ...
            this.#bundler = new (require('./bundler'))( TemplateParser, this.#templates, this.#options );
        }
