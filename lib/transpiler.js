...
module.exports = class Transpiler
{
    ...
    _async_code( nodes )
    {
        ...
        [
            `$$_html.push(( async() =>`
        ,   `{`
        ,       new Transpiler({ nodes: nodes }, this.#options ).code()
        ,   `})());\n`
        ]
        .join('\n');
    }
    ...
    _attributes( attributes )
    {
        for( let attribute of attributes )
        {
            ...
            {
                this.#html_source += ' ' + attribute.name + ( attribute.hasOwnProperty( 'value' ) && attribute.value !== null ? '="' + attribute.value.replace( /\$\{/g, '\\${' ).replace( /\`/g, '\\`' ) + '"' : '' );
            }
        }
    }
    ...
    _style( style )
    {
        if( style.attributes.find( a => a.name === 'bundle' )){ return }
        ...
        let id = Resources.id( 'style', { code: Style.compile( style.source, this.#options.style ).trim() });
    ...
    _script( script )
    {
        if( script.attributes.find( a => a.name === 'bundle' )){ return }
        ...
        if( script.attributes.find( a => a.name === 'instance' ))
        {
            ...
            if( id )
            {
                this.#html_source += `>(function(){${this._vars( script.attributes )}\${$$_script('${id}')}})();</script>`;
            }
            else
            {
                this.#html_source += `></script>`;
        ...
        {
            this.#html_source += `<script` + ( id ? ` id="${id}"` : '' );

            this._attributes( script.attributes.filter( a => !a.name.startsWith( 'var-' )));
    ...
    _template( template )
    {
        ...
        {
            let content_code = new Transpiler({ nodes: template.nodes }, this.#options ).code();
    ...
    {
        this.#html_source += `<@@@ ${destination.name} @@@>`;
    }

    _reference( reference )
    {
        if( reference.nodes && reference.nodes.length )
        {
            //console.log( reference );

            this._flush_html_source( true ); // POSSIBLE ASYNC

            let content = `$$_references.push({ name: '${reference.name}', content: (() => { ${new Transpiler({ nodes: reference.nodes }, this.#options ).code()} })() });\n`;
    ...
    _slot( slot )
    {
        if( slot.nodes && slot.nodes.length )
        {
            //console.log( reference );
