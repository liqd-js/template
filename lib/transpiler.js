...
module.exports = class Transpiler
{
    ...
    _attributes( attributes )
    {
        for( let attribute of attributes )
        {
            ...
            {
                this.#html_source += `\${Object.entries( ${attribute.spread} )${ this._filter( attribute.filter )}.filter(([ name, value ]) => value !== undefined ).map(([ name, value ]) => ' ' + name + ( value !== null ? '="' + value + '"' : '' )).join('')}`;
            }
            else if( attribute.hasOwnProperty( 'expression' ))
            {
                if( attribute.expression.code.includes( 'await' ))
                {
                    ...
                    ,   `return v !== undefined ? ' ${attribute.name}' + ( v !== null ? '="' + v + '"' : '' ) : '';`
                    ]);
                }
                else
                {
                    this.#html_source += `\${(()=>{let v = ${attribute.expression.code}; return v !== undefined ? ' ${attribute.name}' + ( v !== null ? '="' + v + '"' : '' ) : ''})()}`;
                }
            }
            else
            {
    ...
    _script( script )
    {
        if( script.attributes.find( a => a.name === 'instance' ))
        {
            ...
            this.#html_source += `>${script.source.replace(/\\/g,'\\\\')}</script>`;
        }
        else
        {
            this.#html_source += `<script`;
